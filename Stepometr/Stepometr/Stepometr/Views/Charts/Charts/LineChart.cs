using SkiaSharp;
using Stepometer.Controls.Charts.Extensions;
using System.Collections.Generic;
using System.Linq;
using Stepometer.Controls.Charts.Constants;
using Stepometer.Controls.Charts.Helpers;

namespace Stepometer.Controls.Charts.Charts
{
    /// <summary>
    ...
    public class LineChart : PointChart
    {
        ...
        protected override void OnDrawContentEnd(SKCanvas canvas, SKSize itemSize, float origin,
            Dictionary<ChartEntry, SKRect> valueLabelSizes)
        {
                    ...
                    for (int i = 0; i < pps.Value.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(entry.ValueLabel))
                        {
                            ...
                            DrawHelper.DrawLabel(canvas, ValueLabelOrientation, yPositionBehavior, itemSize, point,
                                entry.ValueLabelColor.WithAlpha((byte) (255 * AnimationProgress)), valueLabelSize,
        ...
        private void DrawLineArea(SKCanvas canvas, ChartSerie serie, SKPoint[] points, SKSize itemSize, float origin)
        {
            if (LineAreaAlpha > 0 && points.Length > 1)
            {
                ...
                {
                    using (var shaderX = CreateXGradient(points, serie.Entries, serie.Color,
                        (byte) (LineAreaAlpha * AnimationProgress)))
                    using (var shaderY = CreateYGradient(points, (byte) (LineAreaAlpha * AnimationProgress)))
                    {
                        paint.Shader = EnableYFadeOutGradient
                            ? SKShader.CreateCompose(shaderY, shaderX, SKBlendMode.SrcOut)
                            : shaderX;
        ...
        /// <inheritdoc/>
        protected override void DrawValueLabel(SKCanvas canvas, Dictionary<ChartEntry, SKRect> valueLabelSizes,
            float headerWithLegendHeight, SKSize itemSize, SKSize barSize, ChartEntry entry, float barX, float barY,
            float itemX, float origin)
        {
            if (Series.Count() == 1 && ValueLabelOption == ValueLabelOption.TopOfChart)
                base.DrawValueLabel(canvas, valueLabelSizes, headerWithLegendHeight, itemSize, barSize, entry, barX,
                    barY, itemX, origin);
        }

        /// <inheritdoc/>
        protected override void DrawBar(ChartSerie serie, SKCanvas canvas, float headerHeight, float itemX,
            SKSize itemSize, SKSize barSize, float origin, float barX, float barY, SKColor color)
        {
        ...
        /// <inheritdoc/>
        protected override void DrawBarArea(SKCanvas canvas, float headerHeight, SKSize itemSize, SKSize barSize,
            SKColor color, float origin, float value, float barX, float barY)
        {
            //Area is draw on the OnDrawContentEnd
        }

        private (SKPoint control, SKPoint nextPoint, SKPoint nextControl) CalculateCubicInfo(SKPoint[] points, int i,
            SKSize itemSize)
        {
            var point = points[i];
        ...
        private SKShader CreateXGradient(SKPoint[] points, IEnumerable<ChartEntry> entries, SKColor? serieColor,
            byte alpha = 255)
        {
